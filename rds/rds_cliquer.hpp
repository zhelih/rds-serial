...
template <typename Verifier> void find_max(std::vector<vertex_set>& c, vertex_set& p, const uint* mu, Verifier *v, graph_cliquer* g, std::vector<uint>& res, int level, const std::chrono::time_point<std::chrono::steady_clock> start, const uint time_lim) {
  auto& curC = c[level];
  if(should_exit)
    return;
  if(curC.empty())
...
template <typename Verifier> uint rds(Verifier* v, graph_cliquer* g, algorithm_run& runtime)
{
  #pragma omp parallel
  {
    #pragma omp single
